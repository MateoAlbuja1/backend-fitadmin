ALTER TABLE users ADD COLUMN IF NOT EXISTS document VARCHAR(32);
...
UPDATE users u
SET document = COALESCE(u.document, c.document)
FROM clients c
JOIN roles r ON r.name = 'CLIENTE'
WHERE u.client_id = c.id
  AND u.role_id = r.id
  AND c.notes = 'Registro desde portal publico';
...
WHERE c.notes = 'Registro desde portal publico';
